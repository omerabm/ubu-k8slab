...
hostname | grep -q "master" && ./2.prep_k8s_master.sh || echo "Not a master... Will skip master specific changes"
hostname | grep -q "worker" && ./2.prep_k8s_workers.sh || echo "Not a worker .. Will skip worker specific changes"
